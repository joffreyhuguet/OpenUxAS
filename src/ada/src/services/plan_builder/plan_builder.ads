...
package Plan_Builder with SPARK_Mode is
   ...
   package TaskAssignment_Lists is new SPARK.Containers.Formal.Doubly_Linked_Lists
     (Element_Type => TaskAssignment);

   Max_TaskAssignment_Deque_Depth : constant := 200; -- arbitrary
   subtype TaskAssignment_Ref_Deque is TaskAssignment_Lists.List
     (Capacity => Max_TaskAssignment_Deque_Depth);

   use TaskAssignment_Lists;

   package Int64_TAL_Maps is new SPARK.Containers.Formal.Unbounded_Hashed_Maps
       (Key_Type     => Int64,
        Element_Type => TaskAssignment_Ref_Deque,
        Hash         => Int64_Hash);
   ...
   package SpeedAltPair_Lists is new SPARK.Containers.Formal.Doubly_Linked_Lists
     (Element_Type => SpeedAltPair);

   Max_SpeedAltPair_Deque_Depth : constant := 200; -- arbitrary
   subtype SpeedAltPair_Ref_Deque is SpeedAltPair_Lists.List
     (Capacity => Max_SpeedAltPair_Deque_Depth);

   use SpeedAltPair_Lists;

   package Int64_ROR_Maps is new SPARK.Containers.Formal.Unbounded_Hashed_Maps
       (Key_Type     => Int64,
        Element_Type => SpeedAltPair_Ref_Deque,
        Hash         => Int64_Hash);
